...
# 4. Pubblica l'applicazione (crea un eseguibile standalone)
dotnet publish DidoGest.UI\DidoGest.UI.csproj -c Release -r win-x64 --self-contained true -o .\Publish\DidoGest

# L'applicazione compilata sarà nella cartella Publish\DidoGest
```
...
I file da distribuire sono:
- `DidoGest.exe` - Eseguibile principale
- `DidoGest.dll` - Libreria principale
- Tutte le DLL di dipendenza
- `App.config` - File di configurazione
- `appsettings.json` (se presente)

### Prima Configurazione

1. Modifica `App.config` con i dati della tua azienda
2. Configura il certificato digitale per la firma delle fatture elettroniche
