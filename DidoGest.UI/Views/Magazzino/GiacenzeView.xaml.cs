using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;
using System.Windows;
using System.Windows.Controls;
using DidoGest.Data;
using DidoGest.UI.Services;
using DidoGest.UI.Windows;
using Microsoft.EntityFrameworkCore;

namespace DidoGest.UI.Views.Magazzino;

public partial class GiacenzeView : UserControl
{
    private readonly DidoGestDbContext _context;
    private List<GiacenzaRow> _allRows = new();

    public GiacenzeView()
    {
        InitializeComponent();
        _context = DidoGestDb.CreateContext();

        Loaded += GiacenzeView_Loaded;
    }

    private async void GiacenzeView_Loaded(object sender, RoutedEventArgs e)
    {
        await LoadMagazzini();
        await LoadGiacenze();
    }

    private async Task LoadMagazzini()
    {
        var mags = await _context.Magazzini.OrderBy(m => m.Codice).ToListAsync();
        var items = new List<ComboItem>
        {
            new ComboItem { Id = null, Display = "Tutti i magazzini" }
        };
        items.AddRange(mags.Select(m => new ComboItem { Id = m.Id, Display = $"{m.Codice} - {m.Descrizione}" }));
        CmbMagazzino.ItemsSource = items;
        CmbMagazzino.SelectedIndex = 0;
    }

    private async Task LoadGiacenze()
    {
        try
        {
            var selected = (CmbMagazzino.SelectedItem as ComboItem)?.Id;

            var query = _context.GiacenzeMagazzino
                .AsNoTracking()
                .Include(g => g.Articolo)
                .Include(g => g.Magazzino)
                .AsQueryable();

            if (selected.HasValue)
            {
                query = query.Where(g => g.MagazzinoId == selected.Value);
            }

            var list = await query.ToListAsync();

            _allRows = list.Select(g => new GiacenzaRow
            {
                Id = g.Id,
                ArticoloCodice = g.Articolo?.Codice ?? "",
                ArticoloDescrizione = g.Articolo?.Descrizione ?? "",
                MagazzinoCodice = g.Magazzino?.Codice ?? "",
                Quantita = g.Quantita,
                QuantitaImpegnata = g.QuantitaImpegnata,
                QuantitaDisponibile = g.QuantitaDisponibile,
                UbicazioneFisica = g.UbicazioneFisica
            }).OrderBy(r => r.ArticoloCodice).ThenBy(r => r.MagazzinoCodice).ToList();

            ApplyFilter();
        }
        catch (Exception ex)
        {
            UiLog.Error("GiacenzeView.LoadGiacenze", ex);
            MessageBox.Show($"Errore caricamento giacenze: {ex.Message}", "Errore",
                MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }

    private void ApplyFilter()
    {
        var s = SearchBox.Text?.ToLower() ?? string.Empty;
        var filtered = string.IsNullOrWhiteSpace(s)
            ? _allRows
            : _allRows.Where(r => r.ArticoloCodice.ToLower().Contains(s) || r.ArticoloDescrizione.ToLower().Contains(s)).ToList();

        GiacenzeDataGrid.ItemsSource = filtered;
        TotaleRighe.Text = filtered.Count.ToString();
    }

    private async void CmbMagazzino_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        if (!IsLoaded) return;
        await LoadGiacenze();
    }

    private void SearchBox_TextChanged(object sender, TextChangedEventArgs e)
    {
        if (!IsLoaded) return;
        ApplyFilter();
    }

    private void BtnMagazzini_Click(object sender, RoutedEventArgs e)
    {
        // View embedded: apriamo una finestra semplice che ospita la view dei magazzini
        var win = new Window
        {
            Title = "Magazzini",
            Width = 1000,
            Height = 700,
            WindowStartupLocation = WindowStartupLocation.CenterScreen,
            Content = new MagazziniView()
        };
        win.ShowDialog();

        _ = LoadMagazzini();
        _ = LoadGiacenze();
    }

    private async void GiacenzeDataGrid_MouseDoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
        if (GiacenzeDataGrid.SelectedItem is not GiacenzaRow row) return;
        if (string.IsNullOrWhiteSpace(row.ArticoloCodice)) return;

        var articoloId = await _context.Articoli
            .AsNoTracking()
            .Where(a => a.Codice == row.ArticoloCodice)
            .Select(a => a.Id)
            .FirstOrDefaultAsync();

        if (articoloId <= 0) return;

        var w = new ArticoloEditWindow(_context, articoloId);
        if (w.ShowDialog() == true)
            await LoadGiacenze();
    }

    private sealed class ComboItem
    {
        public int? Id { get; set; }
        public string Display { get; set; } = string.Empty;
        public override string ToString() => Display;
    }

    private sealed class GiacenzaRow
    {
        public int Id { get; set; }
        public string ArticoloCodice { get; set; } = string.Empty;
        public string ArticoloDescrizione { get; set; } = string.Empty;
        public string MagazzinoCodice { get; set; } = string.Empty;
        public decimal Quantita { get; set; }
        public decimal QuantitaImpegnata { get; set; }
        public decimal QuantitaDisponibile { get; set; }
        public string? UbicazioneFisica { get; set; }
    }
}
