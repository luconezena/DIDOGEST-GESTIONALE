...
- **Stato attuale**: disponibile **solo se abilitata** da impostazioni (feature flag)
- Export XML **minimale** (non garantito conforme al tracciato FatturaPA)
- Modalità operative:
   - **Commercialista**: salva l'XML in una cartella configurata
   - **Server**: invia l'XML via HTTP POST a un endpoint esterno (token Bearer)
...
- (Opzionale) Abilita la funzionalità da **Utility → Impostazioni → Fatturazione elettronica**
- Scegli la modalità:
   - **Commercialista** (cartella) oppure
   - **Server** (endpoint esterno)
- Vai su **Fatturazione Elettronica → Fatture Elettroniche**
- Seleziona una fattura e clicca su **Genera/Invia XML**
...
Dal menu **AIUTO → Guida del programma** si apre una finestra che carica il contenuto di `HELP.md`.

Nota per la distribuzione portable:
- `HELP.md` deve stare nella **stessa cartella dell'eseguibile** che lanci.
- Nella distribuzione generata viene copiato sia in `portable\dist\DidoGest-Portable\` sia in `portable\` (accanto a `DidoGest.exe`).
